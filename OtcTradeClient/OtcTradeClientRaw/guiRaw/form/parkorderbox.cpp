...
//extern std::shared_ptr<ParkOrderBox> tradeBox;
extern QOrderMgr* getOrderMgrbyInstrument(tagXTInstrument&);
extern int posDirectionSign(XTPosiDirectionType);

ParkOrderBox* g_parkOrderBox = nullptr;

ParkOrderBox* getParkOrderBox()
{
    if(!g_parkOrderBox)
    {
       // g_parkOrderBox = new ParkOrderBox();
        return g_parkOrderBox;
    }
    return g_parkOrderBox;
}
ParkOrderBox::ParkOrderBox(QWidget* parent /*= nullptr*/)
{
...
ParkOrderBox::~ParkOrderBox()
{

}

void ParkOrderBox::inputCommodity(tagXTInstrument &oCode)
{
    setCurCommodity(oCode);
}

void            ParkOrderBox::initControls()
{
    //右边
...

    {
       ...
       // g1->addWidget(_chkAny, 3, 2);

        _chCloseTodayOrYes = new QCheckBox(tr("平今"));
        g1->addLayout(h2, 3, 0, 1, 2);
      //  g1->addWidget(_chCloseTodayOrYes, 4, 2);
...
void    ParkOrderBox::configControls()
{
    ...
    this->setProperty("name","ParkOrderBox");

}

//选择代码对话框，返回之后
void    ParkOrderBox::onCodeBtnPushed()
{
    SelectCommodity dlg;
...
void    ParkOrderBox::onQuoteClick(int idx)
{
    double dprice =0.0;
...
void      ParkOrderBox::setCurCommodity(tagXTInstrument& oCode)
{
    m_oExCode = oCode;
    memset(&m_oKBDetail, 0, sizeof(m_oKBDetail));
...
//编辑框输入，回车
void            ParkOrderBox::onCodePressReturn()
{
...
void    ParkOrderBox::initAccount(const char* pProductID)
{
    vector<tagOneTradeUser>  arrUser;
    GetConfigModule()->GetTradeUserBy(pProductID, arrUser);
    qDebug() << "ParkOrderBox initAccount number " << arrUser.size();
...
void        ParkOrderBox::initCompleter()
{
...
void   ParkOrderBox::on_buyopenbtn_clicked_or_autolong()    //买入开仓
{
...
void   ParkOrderBox::on_buyclosebtn_clicked()   //买入平仓
{
...
void   ParkOrderBox::on_sellopenbtn_clicked_or_autoshort()   //卖出开仓
{
...
void   ParkOrderBox::on_sellclosebtn_clicked()  //卖出平仓
{
...
int    ParkOrderBox::inputOrderCommon(char  direction, char  offsetflag, int volume)
{
    //[1]检查代码
  ...
  //  _curMgr->InsertOrder(&oField);
    std::string szkey("");
    szkey.append(std::to_string(oField.BrokerID)).append(oField.UserID);
    insertParkOrder(szkey, oField);

    return 1;
}

void            ParkOrderBox::onOrderEvent(OrderEventArgs* e)
{
...
void ParkOrderBox::onAutoChkClicked()
{
...
void ParkOrderBox::closeEvent(QCloseEvent *event)
{
     event->ignore();
     this->hide();
}

std::pair<int, int> ParkOrderBox::getShortOrLongPostionsBy(tagXTInstrument &cur_Code, bool isLong)
{
    int position  = 0;
    int todayPostion = 0;
...
void ParkOrderBox::closePostion(char direction, int vol, int todayPosition)
{
    int iret;
...
int ParkOrderBox::getParkOrderRef()
{
    return m_parkOrderIndex++;
}

void ParkOrderBox::postOrderInfo(const tagXTInputOrderField& inputOrder)
{
    ...
    strcpy(e->orderref, inputOrder.OrderRef);

    e->broker = inputOrder.BrokerID;
    strcpy(e->user, inputOrder.UserID);
    pEventCenter->PostOrderMessage(e);
}

void ParkOrderBox::insertParkOrder(string& userref, tagXTInputOrderField& order)
{
    auto search = m_parkOrders.find(userref);
    if(search != m_parkOrders.end())
    {
        auto& torders = m_parkOrders[userref];
        torders.insert({getParkOrderRef(), order});
    }
    else
    {
        m_parkOrders.insert({userref, {{getParkOrderRef(), order}}});
    }
    postOrderInfo(order);
}

void ParkOrderBox::removeParkOrder(string& userref, int orderIndex)
{
    auto search = m_parkOrders.find(userref);
    if(search != m_parkOrders.end())
    {
        auto& torders = m_parkOrders[userref];
        auto temp = torders[orderIndex];
        torders.erase(orderIndex);
        postOrderInfo(temp);
    }
}

ParkOrders &ParkOrderBox::getParkOrders()
{
    return m_parkOrders;
}

/*void ParkOrderBox::insertParkOrder(tagXTInputOrderField &)
{
